...
public class UnbundledArtSummaryProcessor {
    ...
    public Map<String, Object> process(String month, String year, Long facilityId) {
        ...
        try {
            ...
            //query = "SELECT DISTINCT patient_id, gender, DATEDIFF(YEAR, date_birth, '" + reportingDateBegin + "') AS age FROM patient WHERE facility_id = " + id + " AND date_registration <= '" + reportingDateEnd + "' AND  current_status IN (" + Constants.ClientStatus.ON_TREATMENT + ") OR (current_status IN (" + Constants.ClientStatus.LOSSES + ") AND date_current_status > '" + reportingDateEnd + "')";
            preparedStatement = jdbcUtil.getStatement(query);
            resultSet = preparedStatement.executeQuery();
            while (resultSet.next()) {
            ...
            //query = "SELECT DISTINCT patient_id, gender, DATEDIFF(YEAR, date_birth, '" + reportingDateBegin + "') AS age FROM patient WHERE facility_id = " + id + " AND current_status IN (" + Constants.ClientStatus.ON_TREATMENT + ") OR (current_status IN (" + Constants.ClientStatus.LOSSES + ") AND date_current_status > '" + reportingDateEnd + "') AND date_started IS NOT NULL AND date_started <= '" + reportingDateEnd + "'";
            preparedStatement = jdbcUtil.getStatement(query);
            resultSet = preparedStatement.executeQuery();
            while (resultSet.next()) {
                long patientId = resultSet.getLong("patient_id");
