package org.fhi360.lamis.security;
...
public final class SecurityUtils {
    ...
    /**
     * If the current user has a specific authority (database role).
     * <p>
    ...
    public static Facility getCurrentFacility() {
        return getCurrentUserLogin().map(username -> {
            UserRepository userRepository = ContextProvider.getBean(UserRepository.class);
            return userRepository.findByUsername(username).getFacility();
        }).orElse(null);
    }
}
